...
import { replacePointageSnapshot } from '../services/pointageSnapshotStore';
...
export default function DailyPointageImport({ employees, baseEmployees = employees, snapshot, onSaved, loading, translate, locale, productionLabels, productionModTarget, onProductionModTargetChange }) {
  ...
  return <div className="daily-import">
    <DailyAttendanceOverview day={attendance[0]} dates={dates} analysisDate={analysisDate} onDateChange={setSelectedDay}
      ...
      busy={busy} importDisabled={loading || !employees.length} fileName={data?.fileName} importedAt={snapshot?.generatedAt} message={message} translate={translate} locale={locale} productionLabels={productionLabels} />
