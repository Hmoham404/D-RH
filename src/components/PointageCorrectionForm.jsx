  ...
  return <form className="pointage-correction" onSubmit={submit}>
    ...
    <fieldset disabled={saving || disabled}>
      <div className="pointage-correction__hours">
        <label>Heure d’entrée<input type="time" required value={entry} onChange={(event) => setEntry(event.target.value)} /></label>
        <label>Heure de sortie<input type="time" required value={exit} onChange={(event) => setExit(event.target.value)} /></label>
      </div>
      <small>La sortie doit être après l’entrée, dans la même journée.</small>
