  ...
  return <div className="daily-import">
    ...
    <form className="delete-zone" onSubmit={clearPointage}>
      <div className="delete-zone__copy">
        <strong>Vider la base pointage</strong>
        <span>Supprime le pointage courant. Les prochains imports repartiront sans reprendre les anciennes journees.</span>
        {clearError && <p role="alert">{clearError}</p>}
      </div>
      <div className="delete-zone__actions">
        <input className="delete-zone__input" type="password" autoComplete="off" autoCapitalize="none" spellCheck={false} placeholder="Code RH" value={clearCode} onChange={(event) => setClearCode(event.target.value)} disabled={busy || correcting || clearing} />
        <button className="danger-button" type="submit" disabled={busy || correcting || clearing || !clearCode.trim()}>{clearing ? 'Vidage...' : 'Vider la base'}</button>
      </div>
    </form>
